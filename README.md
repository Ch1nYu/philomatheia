...
Philomatheia turns a learning goal into a visible knowledge map, teaches through small adaptive loops, records what the learner can actually demonstrate, and resumes from an exact checkpoint across sessions. It is designed for Codex and other hosts that support the open Agent Skills format.
...
Ask the built-in Skill Installer to install this repository:
...
The scripts install only the runtime skill files under `$HOME/.agents/skills/philomatheia`. They refuse to replace an existing installation unless `-Update` or `--update` is explicitly supplied. See [INSTALL.md](INSTALL.md) for preview and manual options.

### Start a learning project

Open Codex in an empty folder dedicated to one learning project, then prompt:

```text
$philomatheia I want to learn statistics well enough to read machine-learning papers critically. I have four hours per week. Diagnose my current level, then propose the first useful knowledge-map route for my approval.
```

Other examples:

```text
$philomatheia Teach me practical Japanese for ordering food safely with a severe peanut allergy. Separate language practice from claims about food safety.
```

```text
$philomatheia Resume this learning project from its exact checkpoint. Begin with one short recall question and do not repeat completed material.
```

The skill may also activate automatically when the request matches the learning-oriented description in [SKILL.md](SKILL.md).
